...
class MyFrame(Frame):
    ...
    def exitButtonClick(self):
        if (messagebox.askokcancel("OK to close?","Close application?")):
            # call the destroy method and exit the process
            try:
                self.parent.destroy()
            except Exception:
                pass
    ...
    def calcButtonClick(self):
        # Validate inputs: non-empty, numeric and sensible ranges
        amt_str = self.txtAmount.get().strip()
        rate_str = self.txtRate.get().strip()
        dur_str = self.txtDuration.get().strip()

        if not amt_str or not rate_str or not dur_str:
            messagebox.showerror("Input Error", "Please enter Amount, Rate and Duration.")
            return
        ...
        try:
            ...
            messagebox.showerror("Input Error", "Amount, Rate and Duration must be numeric values.")
            return
        ...
        if amt <= 0:
            ...
            return
        if rate < 0:
            messagebox.showerror("Input Error", "Rate cannot be negative.")
            return
        if dur <= 0:
            messagebox.showerror("Input Error", "Duration must be greater than 0.")
            return
        ...
        try:
            ...
            messagebox.showerror("Calculation Error", f"Unable to calculate payment: {e}")
            return

        # Update readonly fields (toggle state temporarily)
